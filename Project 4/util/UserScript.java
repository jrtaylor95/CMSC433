 ...
 * AccessRequest / AccessRelease / ManagementRequest objects.  The idea is that each request in step
 * should be sent, then the corresponding response received, before the next step is
 * executed.
 * 
 * @author Rance Cleaveland, Jonathan Hansford
 *
 */
public class UserScript {
	...
	/**
	 * Parses a string into a script.  The string must have the following format.
	 * - Each step is on a separate line.
	 * - Each step consists of a non-empty sequence of statements, separated by "|"
	 * - Each statement has one of the following forms (no parens, "/" indicates possible options)
	 *   (write/read)-(request-b/n) string
	 *   (write/read)-release string
	 *   (enable/disable/add/remove) string
	 * where "string" is the name of a resource.  The "-b" and "-n" indicate blocking and
	 * non-blocking requests, respectively
	 * 
	 * Example requests:
	 * - read-request-n Scanner_0
	 * - write-release 	Printer_0
	 * 
	 * @param script	String to be turned into script
	 * @return			UserScript object
	 */
	public static UserScript fromString (String script) {
		...
		// Process each line, removing empty lines as they are encountered.
		for (String line : lines) {
			...
			// Split line into individual statements and process each statement
			...
			for (String statement : statements) {				
				...
				// Identify command, resource name
				...
				// Construct appropriate object corresponding to command and and resource
				Object action;
				if (command.equalsIgnoreCase("write-request-n")) {
					...
					action = new ManagementRequest(resource_name, ManagementRequestType.ADD);
				} else if (command.equalsIgnoreCase("remove")) {
					action = new ManagementRequest(resource_name, ManagementRequestType.REMOVE);
				} else {
					throw new IllegalArgumentException("Illegal command: " + command + "\n\ton the line: " + line);
				}
				// Add statement to current step being constructed
				thisLine.add(action);
			}
			// Add completed step to list of steps
			result.add(thisLine);
		}
